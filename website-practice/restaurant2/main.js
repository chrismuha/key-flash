  ...
  function setMenuLaunchDisplayLabel(button, label) {
    if (!button || !label) return;
    const labelEl = button.querySelector('.menu-launch-label');
    if (labelEl) {
      labelEl.textContent = label;
      return;
    }
    const textNode = Array.from(button.childNodes).find((node) => node && node.nodeType === Node.TEXT_NODE && String(node.textContent || '').trim());
    if (textNode) {
  ...
  function applyOwnerConfigToDom() {
    Object.entries(SECTION_LABELS).forEach(([section, label]) => {
      ...
      document.querySelectorAll(`.menu-launch[data-target="${section}"]`).forEach((button) => {
        setMenuLaunchDisplayLabel(button, label);
      });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (body.classList.contains('order-type')) {
      ...
      if (businessLocationTrigger && businessLocationOptions && businessLocationEntries.length) {
        ...
        const businessLocationOptionButtons = businessLocationEntries.map((location) => {
          ...
          option.type = 'button';
          option.className = 'business-location-option';
          option.setAttribute('role', 'option');
          option.dataset.value = location.id;
          option.textContent = location.label;
          businessLocationOptions.appendChild(option);
          return option;
        });
    ...
    if (body.classList.contains('page3')) {
      ...
      if (page3FooterResetBtn) {
        page3FooterResetBtn.addEventListener('click', (event) => {
          event.preventDefault();
          openCustomConfirm('Reset all selections and return to the menu builder?', (confirmed) => {
            if (!confirmed) return;
            try { localStorage.removeItem(STORAGE_KEYS.orderItems); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.activeSections); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.ingredients); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.quantities); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.quantitiesSections); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.sectionNotes); } catch { /* ignore */ }
            try { localStorage.removeItem(STORAGE_KEYS.manualDisabledSections); } catch { /* ignore */ }
            try { localStorage.setItem(STORAGE_KEYS.pizzaSize, DEFAULT_PIZZA_SIZE); } catch { /* ignore */ }
            window.location.href = 'page2.html';
          }, { restoreFocus: false });
        });
      }
