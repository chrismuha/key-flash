...
/* === Appended by ChatGPT: Robust pill-arrow-only guard ===
   This guard blocks non-arrow clicks on .menu-launch pills when
   window.pillArrowOnly is truthy. It runs early (pointerdown capture)
   so it prevents overlays that open on earlier events.
   It still allows interaction with inputs, labels, buttons, anchors,
   selects, textareas and contenteditable elements inside the pill.
*/
(function installPillArrowOnlyGuard() {
  const interactiveSelector = 'input, select, textarea, button, a[href], label, [contenteditable="true"]';

  function pillArrowOnlyEnabled() {
    // Use the global variable if present; default to false otherwise.
    try { return !!window.pillArrowOnly; } catch (err) { return false; }
  }

  function shouldBlockEvent(e) {
    if (!pillArrowOnlyEnabled()) return false; // respect the setting
    const el = e.target;
    if (!el || !el.closest) return false;
    // find the pill element (closest ancestor with menu-launch[data-target])
    const pill = el.closest('.menu-launch[data-target]');
    if (!pill) return false; // not inside a menu pill

    // Allow clicks if they originate from interactive controls inside the pill
    if (el.closest(interactiveSelector)) return false;

    // Allow if the arrow itself (or a child of it) was the target
  ...
  // Capture pointerdown early and block (prevents later click handlers / navigation)
  ['pointerdown', 'mousedown', 'touchstart'].forEach((evtName) => {
    document.addEventListener(evtName, (e) => {
      try {
        ...
        console.error('pill-arrow-only guard error:', err);
      }
    }, true); // use capture so we run before other listeners
  });

  // keep existing click guard as a fallback (in case some flows only generate click)
  document.addEventListener('click', (e) => {
    try {
      if (shouldBlockEvent(e)) {
        e.preventDefault();
      ...
      console.error('pill-arrow-only guard error (click):', err);
    }
  }, true);
})();
